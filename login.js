...
gapi.auth.checkSessionState({client_id: ''}, googleSessionStatus);
}
gp_login = function () {
var myParams = {
    'clientid': $gClient, //You need to set client id
...
if (result['status']['signed_in']) {
    ...
    gapi.auth.checkSessionState({client_id: $gClient}, true);
...
gapi.client.load('plus', 'v1', function () {
    ...
    request.execute(function (resp) {
        ...
        if (json.email == '') {
            toastrNotification('Failed to get email from google, Please login using form', 'error');
            return false;
        }
